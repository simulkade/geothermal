...
skin_factor = 3.0 # well skin factor
...
for t_ind in 2:length(t_step)
  ...
  rwa=well_diameter/2*exp(-skin_factor)
  WI_inj = 2*π*perm_val*thick_res/(mu_val.value[ind_inj+1 ...]*log(r_eq/(rwa)))
  WI_prod = 2*π*perm_val*thick_res/(mu_val.value[ind_prod+1 ...]*log(r_eq/(well_diameter/2)))
  p_inj[t_ind]  = p_val.value[ind_inj+1 ...]+flow_inj/WI_inj # change this line to include skin factor
...
p_inj[1] = p_inj[2]
p_out[1] = p_out[2]
df1 = DataFrame(T_s = t_step, dp_Pa = dp_res, T_K = T_out, p_inj_Pa = p_inj, p_out_Pa = p_out)
DataFrames.writetable("Q-$flow_m3_h-L-$flow_m3_h-k$perm_D-skin$skin_factor.csv", df1)
figure(figsize=(8,2))
