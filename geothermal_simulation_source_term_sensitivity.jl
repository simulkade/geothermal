...
# legend()
# run it in a loop:
df_res = DataFrame(q_m3_h = [], k_D = [], L_m = [], csv_name =[])
flow_range = [50.0, 100.0, 150.0, 200.0, 250.0] # [m3/h]
perm_range = [20.0, 50.0, 100.0, 200.0, 500.0, 1000.0]/1000.0 # [D]
L_range    = [500.0, 650.0, 800.0, 900.0, 1000.0] # [m]
for flow_m3_h in flow_range
  for perm_D in perm_range
    for L_res in L_range
      ...
      BCp.left.a[:] = 0.0
      BCp.left.b[:] = 1.0
      BCp.left.c[:] = p_res # this makes the convergence easier and does not affect the physics much
      # BCp.right.a[:] = 0.0
      # BCp.right.b[:] = 1.0
      # BCp.right.c[:] = p_res
